/**
 * SPDX-FileCopyrightText: (c) 2026 Liferay, Inc. https://liferay.com
 * SPDX-License-Identifier: LGPL-2.1-or-later OR LicenseRef-Liferay-DXP-EULA-2.0.0-2023-06
 */

package com.liferay.customer.util;

import com.liferay.customer.constants.AccountRoleConstants;
import com.liferay.customer.constants.KoroneikiContactRoleConstants;
import com.liferay.headless.admin.user.client.dto.v1_0.AccountBrief;
import com.liferay.headless.admin.user.client.dto.v1_0.AccountRole;
import com.liferay.headless.admin.user.client.dto.v1_0.RoleBrief;
import com.liferay.headless.admin.user.client.dto.v1_0.UserAccount;
import com.liferay.osb.koroneiki.phloem.rest.client.dto.v1_0.ContactRole;
import com.liferay.portal.kernel.util.ArrayUtil;
import com.liferay.portal.kernel.util.Validator;

import java.util.ArrayList;
import java.util.Collection;
import java.util.Collections;
import java.util.HashMap;
import java.util.List;
import java.util.Map;

/**
 * @author Felipe Franca
 */
public class AccountRoleUtil {

	public static Map<String, Long> getContactAccountRoleIdMap(
		Collection<AccountRole> accountRoles) {

		Map<String, Long> accountRoleIdMap = new HashMap<>();

		for (AccountRole accountRole : accountRoles) {
			if (ArrayUtil.contains(
					AccountRoleConstants.NAMES_CONTACT_ACCOUNT_ROLES,
					accountRole.getName())) {

				accountRoleIdMap.put(
					accountRole.getName(), accountRole.getId());
			}
		}

		return accountRoleIdMap;
	}

	public static List<Long> getContactAccountRoleIds(
		Map<String, Long> accountRoleIdMap, ContactRole[] contactRoles) {

		List<Long> accountRoleIds = new ArrayList<>();

		if (ArrayUtil.isEmpty(contactRoles)) {
			return accountRoleIds;
		}

		for (ContactRole contactRole : _filterContactRoles(contactRoles)) {
			Long accountRoleId = _getAccountRoleId(
				accountRoleIdMap, contactRole.getName());

			if (accountRoleId != null) {
				accountRoleIds.add(accountRoleId);
			}
		}

		return accountRoleIds;
	}

	public static List<Long> getCurrentAccountRoleIds(
		UserAccount userAccount, String accountExternalReferenceCode,
		Collection<Long> accountRoleIds) {

		if (userAccount == null) {
			return Collections.emptyList();
		}

		AccountBrief[] accountBriefs = userAccount.getAccountBriefs();

		if (ArrayUtil.isEmpty(accountBriefs)) {
			return Collections.emptyList();
		}

		List<Long> currentAccountRoleIds = new ArrayList<>();

		for (AccountBrief accountBrief : accountBriefs) {
			if (!accountExternalReferenceCode.equals(
					accountBrief.getExternalReferenceCode())) {

				continue;
			}

			RoleBrief[] roleBriefs = accountBrief.getRoleBriefs();

			if (ArrayUtil.isEmpty(roleBriefs)) {
				continue;
			}

			for (RoleBrief roleBrief : roleBriefs) {
				Long roleId = roleBrief.getId();

				if (accountRoleIds.contains(roleId)) {
					currentAccountRoleIds.add(roleId);
				}
			}
		}

		return currentAccountRoleIds;
	}

	private static ContactRole[] _filterContactRoles(
		ContactRole[] contactRoles) {

		List<ContactRole> filteredContactRoles = new ArrayList<>();

		for (ContactRole contactRole : contactRoles) {
			if (ArrayUtil.contains(
					KoroneikiContactRoleConstants.NAMES_KORONEIKI_CONTACT_ROLES,
					contactRole.getName())) {

				filteredContactRoles.add(contactRole);
			}
		}

		return filteredContactRoles.toArray(new ContactRole[0]);
	}

	private static Long _getAccountRoleId(
		Map<String, Long> accountRoleIdMap, String contactRoleName) {

		if (Validator.isNull(contactRoleName)) {
			return null;
		}

		if (contactRoleName.equals(
				KoroneikiContactRoleConstants.NAME_CRITICAL_INCIDENT_CONTACT)) {

			return accountRoleIdMap.get(
				AccountRoleConstants.NAME_CRITICAL_INCIDENT_CONTACT);
		}
		else if (contactRoleName.equals(
					KoroneikiContactRoleConstants.
						NAME_CUSTOMER_EXPERIENCE_MANAGER)) {

			return accountRoleIdMap.get(
				AccountRoleConstants.NAME_CUSTOMER_EXPERIENCE_MANAGER);
		}
		else if (contactRoleName.equals(
					KoroneikiContactRoleConstants.NAME_DATA_BREACH_CONTACT)) {

			return accountRoleIdMap.get(
				AccountRoleConstants.NAME_DATA_BREACH_CONTACT);
		}
		else if (contactRoleName.equals(
					KoroneikiContactRoleConstants.NAME_PARTNER_MANAGER)) {

			return accountRoleIdMap.get(
				AccountRoleConstants.NAME_PARTNER_MANAGER);
		}
		else if (contactRoleName.equals(
					KoroneikiContactRoleConstants.
						NAME_PARTNER_MARKETING_USER)) {

			return accountRoleIdMap.get(
				AccountRoleConstants.NAME_PARTNER_MARKETING_USER);
		}
		else if (contactRoleName.equals(
					KoroneikiContactRoleConstants.NAME_PARTNER_MEMBER)) {

			return accountRoleIdMap.get(
				AccountRoleConstants.NAME_PARTNER_MEMBER);
		}
		else if (contactRoleName.equals(
					KoroneikiContactRoleConstants.NAME_PARTNER_SALES_USER)) {

			return accountRoleIdMap.get(
				AccountRoleConstants.NAME_PARTNER_SALES_USER);
		}
		else if (contactRoleName.equals(
					KoroneikiContactRoleConstants.
						NAME_PARTNER_TECHNICAL_USER)) {

			return accountRoleIdMap.get(
				AccountRoleConstants.NAME_PARTNER_TECHNICAL_USER);
		}
		else if (contactRoleName.equals(
					KoroneikiContactRoleConstants.
						NAME_SECURITY_INCIDENT_CONTACT)) {

			return accountRoleIdMap.get(
				AccountRoleConstants.NAME_SECURITY_INCIDENT_CONTACT);
		}
		else if (contactRoleName.equals(
					KoroneikiContactRoleConstants.NAME_SOLUTION_ARCHITECT)) {

			return accountRoleIdMap.get(
				AccountRoleConstants.NAME_SOLUTION_ARCHITECT);
		}
		else if (contactRoleName.equals(
					KoroneikiContactRoleConstants.NAME_SUPPORT_ADMINISTRATOR)) {

			return accountRoleIdMap.get(
				AccountRoleConstants.NAME_ACCOUNT_ADMINISTRATOR);
		}
		else if (contactRoleName.equals(
					KoroneikiContactRoleConstants.NAME_SUPPORT_REQUESTER)) {

			return accountRoleIdMap.get(AccountRoleConstants.NAME_REQUESTER);
		}
		else if (contactRoleName.equals(
					KoroneikiContactRoleConstants.NAME_SUPPORT_USER)) {

			return accountRoleIdMap.get(
				AccountRoleConstants.NAME_ACCOUNT_MEMBER);
		}

		return null;
	}

}